...
public class Test_DataSource extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			DataSource ds = (DataSource) ctx.lookup("java:comp/env/jdbc/cia102g2db");
			Connection conn = ds.getConnection();

			if (conn != null) {
				out.println("Got Connection: " + conn.toString());
				Statement stmt = conn.createStatement();
				ResultSet rs = stmt.executeQuery("select * from shop_product_pic");
				while (rs.next()) {
					out.println("prod_pic_no = " + rs.getString(1));
				}
				conn.close();
			}
